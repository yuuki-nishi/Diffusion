        ...
        for i in range(num):
          ...
          while True:
            if isVoice:
              ...
              lines2=fileterclass2(lines,l)
              wavfilename=np.random.choice(lines2)
              wavrate,wav=wavfile.read(voice_list_dir+"/"+wavfilename)
              #kind=l
              speakers.add(l)
              #continue
            else:
              noiselist=glob.glob(noise_dir+"/{}_bg/{}/*wav".format(l,classname))
              if len(noiselist) == 0:
                break
              #noiselist2=fileterclass2(noiselist,l)
