...
class Mixer(object):
  ...
  def __init__(self, output_root, fg_root, bg_root, allow_same_label=False,
               num_train=200, num_validation=50, num_eval=50, random_seed=123, max_fevent=3):
    self.fg_root = fg_root
    print("fg_root : {}".format(fg_root))
  ...
  def get_file_list(self, subset, style):
    ...
    arglist=['400', '002', '001']
    file_list = [i for i in file_list if os.path.basename(i)[0:3] in arglist]
  ...
  def generate_example(self, subset, n, num_examples,
    ...
    def add_events(num_events, file_list, ind_file, used_labels, add_event_fn,
      ...
      for _ in range(num_events):
        ...
        while label is None or label in used_labels:
          ...
          print("filename : {}".format(source_file))
    ...
    for i in range(len(names)):
      tmpname=names[i]
      basename_without_ext = os.path.splitext(os.path.basename(tmpname))[0]
      print("processed name : {}".format(basename_without_ext))
      names[i]=basename_without_ext#名前だけ取得
    sourcesroot=os.path.join(self.output_root, src_dir)
    print("root : {}".format(sourcesroot))
    #wavファイルの名前を、元のファイル名があるように変える
    os.rename(os.path.join(sourcesroot,'./background0_sound.wav'), os.path.join(sourcesroot,'./{}_background0_sound.wav'.format(names[0])))
    for i in range(num_fg_events):
      os.rename(os.path.join(sourcesroot,'./foreground{:d}_sound.wav'.format(i)), os.path.join(sourcesroot,'./{}_foreground{:d}_sound.wav'.format(names[i+1],i)))

    bg_file = os.path.join(src_dir, '{}_background0.wav'.format(names[0]))
    fg_files = [os.path.join(
        src_dir, '{}_foreground{:d}.wav'.format(names[i+1],i))
                for i in range(num_fg_events)]
...
def main():
  ...
  parser.add_argument(
      ...
      type=int)
  parser.add_argument(
      '-mn', '--max_fevent', help='max_foreground_eventnum seed.', required=False, default=3,
      type=int)
