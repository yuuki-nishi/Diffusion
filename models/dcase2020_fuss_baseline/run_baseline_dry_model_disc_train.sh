...
#${VoiceExec_name}=${VoiceExec_name} #allの時用
ParamSize=$2
variance_omega=$3
#VoiceExec_name=${VoiceExec_name}_${ParamSize}
Final_batch=4 #組み合わせの時用
#uはunitのu
OUTPUT_DIR=${MODEL_DIR}/baseline_dry_train_u64_mixup_newdisc/fb_${Final_batch}/${VoiceExec_name}
#rm -rf ${OUTPUT_DIR}
mkdir -p ${OUTPUT_DIR}
Preprocessed_data=${SCRIPT_PATH}/../../data/Noise_Voice_Preprocessed
#NeedAll=""

#if [ $NoiseKind  = "all" ]; then
#  NeedAll="-all"
#  NoiseKind=""
#fi


python3 ${SCRIPT_PATH}/train_model_disc.py -dd=${DEV_DATA_DIR} -pdd=${Preprocessed_data} \
-md=${OUTPUT_DIR} -cf=${ConfigPath} -sz=${ParamSize} -vari_omega=${variance_omega} -fb=${Final_batch} \
-mixup
