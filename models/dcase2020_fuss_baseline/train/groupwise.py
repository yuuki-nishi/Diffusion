...
def apply_withlabel(loss_fns: typing.Dict[str, typing.Callable[..., typing.Any]],
          loss_fns_label: typing.Dict[str, typing.Callable[..., typing.Any]],
          signal_names: typing.List[str],
          reference: tf.Tensor,
          estimate: tf.Tensor,
          labels: tf.Tensor,
  ...
  loss = tf.zeros(shape=reference.shape[:2], dtype=tf.float32)
  permuted_estimates = tf.zeros_like(reference)
  permuted_estimates_label = tf.zeros_like(labels, dtype=permuted_estimates.dtype)

  # For each kind of signal, e.g. 'speech', 'noise', gather subsets of reference
  # and estimate, apply loss function and scatter-add into the loss tensor.
  for (name,loss_fn),(_,loss_fn_label) in zip(loss_fns.items(),loss_fns_label.items()):
    #name = loss_fns.items()[i]
    #loss_fn = loss_fns.items()[i]
    #oss_fn_label = loss_fns_label.items()[i]
    idxs = [idx for idx, value in enumerate(signal_names) if value == name]
    idxs_0 = tf.tile(
        tf.expand_dims(tf.range(batch), 1),
        [1, len(idxs)])
    ...
    reference_key = tf.gather_nd(reference, idxs_nd)
    estimate_key = tf.gather_nd(estimate, idxs_nd)
    reference_label_key = tf.gather_nd(labels, idxs_nd)
    estimate_label_key = tf.gather_nd(probs, idxs_nd)
    loss_fn = permutation_invariant.wrap_label(
        loss_fn,
        loss_fn_label,
        enable=name in permutation_invariant_losses)
    loss_key, permuted_estimates_key,permuted_estimates_label_key = loss_fn(reference_key, estimate_key,reference_label_key,estimate_label_key)
    
    loss = tf.tensor_scatter_add(loss, idxs_nd, loss_key)
    permuted_estimates = tf.tensor_scatter_add(
        permuted_estimates, idxs_nd, permuted_estimates_key)
    permuted_estimates_label = tf.tensor_scatter_add(
        permuted_estimates_label, idxs_nd, permuted_estimates_label_key)

  return loss, permuted_estimates,permuted_estimates_label,idxs_nd
