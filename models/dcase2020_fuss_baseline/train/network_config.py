...
class DenseLayer(object):
  ...
  # clip params
  kernel_constraint = attr.attrib(type=typing.Text, default=tf.keras.constraints.MinMaxNorm())
...
class TimeConvLayer(object):
  ...
  separable = attr.attrib(type=bool, default=True)#Trueにするとdepthwiseになる
...
class TDCNBlock(object):
  ...
  # 1-1Time convolution layer
  pwisetclayer = attr.attrib(type=typing.Optional[TimeConvLayer], default=None)
  # Normalization and activation following tclayer
  normact2 = attr.attrib(
      type=typing.Optional[NormAndActivationLayer], default=None)
  normact = attr.attrib(#どうせ共通だから
      type=typing.Optional[NormAndActivationLayer], default=None)
...
def improved_tdcn(depth_multiplier=1):
  ...
  netcfg.prototype_block = [TDCNBlock(
      ...
      pwisetclayer=TimeConvLayer(separable=False))]
  return netcfg
def improved_tdcn_28(depth_multiplier=1):
  """Build ImprovedTDCN object for improved_tdcn."""
  normact = NormAndActivationLayer(
      norm_layer=NormLayer(
          norm_type='global_layer_norm', bin_wise=True),
      activation='prelu')
  netcfg = ImprovedTDCN(
      block_prototype_indices=([0] * (28 * depth_multiplier)),
      block_dilations=([1, 2, 4, 8, 16, 32, 64] * (4 * depth_multiplier)),
      skip_residue_connection_from_input_of_block=[0, 0, 0, 8, 8, 16],
      skip_residue_connection_to_input_of_block=[8, 16, 24, 16, 24, 24],
      scale_tdcn_block='exponential')
  dense_biased = DenseLayer(scale=1.0, use_bias=True)
  ...
  return netcfg
# i wrote
def small_improved_tdcn(depth_multiplier=1):
  """Build ImprovedTDCN object for improved_tdcn."""
  normact = NormAndActivationLayer(
      norm_layer=NormLayer(
          norm_type='global_layer_norm', bin_wise=True),
      activation='prelu')
  netcfg = ImprovedTDCN(
      block_prototype_indices=([0] * (24 * depth_multiplier)),#これがブロックの数に当たる
      block_dilations=([1, 2, 4, 8, 16, 32, 64] * (4 * depth_multiplier)),
      skip_residue_connection_from_input_of_block=[0, 0, 0, 8, 8, 16],
      skip_residue_connection_to_input_of_block=[8, 16, 24, 16, 24, 24],
      scale_tdcn_block='exponential')
