...
def dense_layer(x, config):
  ...
  dense_output = tf.keras.layers.Dense(
      ...
      bias_initializer=bias_initializer).apply(x)
      #name='dense').apply(x)
  if config.scale >= 0.0:
...
def time_convolution_layer(x, config):
  ...
  if config.separable:#こっちしかいないっぽい
...
def norm_and_activation_layer(x, config):
  ...
  if config.norm_after_act:#defaultではこっち
...
def tdcn_block(x, config):
  ...
  with tf.variable_scope('tdcn_block'):
    ...
    dense2_config = update_config_from_kwargs(
        config.dense2, num_outputs=config.bottleneck, scale=config.scale,#これが0.9^lとなている
...
def improved_tdcn(input_activations, config):
  ...
  with tf.variable_scope('improved_tdcn'):
    for block in range(num_blocks):
      proto_block = config.block_prototype_indices[block]#これ全部0
...
def dense_cliped_layer(x, config):
  """Configurable dense layer.
  ...
  dense_output = tf.keras.layers.Dense(
      ...
      name='dense',
      kernel_constraint=config.kernel_constraint).apply(x)
  if config.scale >= 0.0:
    dense_output = scale_layer(dense_output,
                               scope='dense_scale',
                               init_param=config.scale)
