...
ParamSize=$2
variance_omega=$3
VoiceExec_name=${VoiceExec_name}_${ParamSize}
OUTPUT_DIR=${MODEL_DIR}/baseline_dry_evaluate_disc_lossvar/variomega_${variance_omega}/${VoiceExec_name}
echo ${OUTPUT_DIR}
mkdir -p ${OUTPUT_DIR}
MODEL_DIR=${MODEL_DIR}/baseline_dry_train_disc_lossvar/variomega_${variance_omega}/${VoiceExec_name}
echo model_dir : ${MODEL_DIR}
#MODEL_DIR=${MODEL_DIR}/baseline_dry_train_disc/Disc_all #これは全ての種類でのノイズで訓練したパラメタを使った時用
...
#python3 ${SCRIPT_PATH}/evaluate_disc.py -cp=${MODEL_DIR} -mp=${MODEL_DIR}/inference.meta -dp=${DISC_DATA_DIR}${NeedSlash}${NoiseKind}/eval_labels.txt -op=${OUTPUT_DIR} -vn=${VoiceExec_name}
python3 ${SCRIPT_PATH}/evaluate_disc.py -cp=${MODEL_DIR} -mp=${MODEL_DIR}/inference.meta \
-dp=${DEV_DATA_DIR} -op=${OUTPUT_DIR} \
-vn=${VoiceExec_name}  -cf=${ConfigPath} 
end=$(date +"%T")
